...
function Feature({ feature, isActive, className, ...props }) {
  return (
    <div className={`${className} ${!isActive && 'opacity-75 hover:opacity-100'}`} {...props}>
      <div className={`w-9 rounded-lg ${isActive ? 'bg-blue-600' : 'bg-slate-500'}`}>
        <svg aria-hidden='true' className='h-9 w-9' fill='none'>
          <feature.icon />
        </svg>
      </div>
      <h3 className={`mt-6 text-sm font-medium ${isActive ? 'text-blue-600' : 'text-slate-600'}`}>{feature.name}</h3>
            ...
            <div className='-mx-5 flex'>
              {features.map((feature, featureIndex) => (
                <Tab.Panel
                  static
                  key={feature.name}
                  className={`px-5 transition duration-500 ease-in-out [&:not(:focus-visible)]:focus:outline-none ${
